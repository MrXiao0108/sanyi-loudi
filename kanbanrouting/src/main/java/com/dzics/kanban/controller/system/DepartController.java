...
import com.dzics.kanban.service.BusinessDepartService;
...
//    @OperLog(operModul = "站点管理", operType = OperType.QUERY, operDesc = "列表", operatorType = "后台")
    ...
    public Result<ResDepart> delDepart(
            @PathVariable("departId") @ApiParam(value = "站点id", required = true) Long departId,
            @RequestHeader(value = "jwt_token", required = false) @ApiParam(value = "token令牌", required = true) String tokenHdaer,
            @RequestHeader(value = "sub", required = false) @ApiParam(value = "用户账号", required = true) String sub) {
        return businessDepartService.delDepart(departId, sub);
    }
