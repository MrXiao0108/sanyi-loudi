...
import com.dzics.kanban.model.vo.user.DisableEnabledUser;
import com.dzics.kanban.model.vo.user.ResetUser;
import com.dzics.kanban.model.vo.user.SelUser;
import com.dzics.kanban.model.vo.user.UpdateUser;
...
//    @OperLog(operModul = "用户管理", operType = OperType.QUERY, operDesc = "账号列表", operatorType = "后台")
    ...
    public Result delUser(@PathVariable("userId") Long userId,
                          ...
                          @RequestHeader(value = "jwt_token", required = false) @ApiParam(value = "token令牌", required = true) String tokenHdaer,
                          @RequestHeader(value = "sub", required = false) @ApiParam(value = "用户账号", required = true) String sub) {
        Result result = businessUserService.delUser(userId, usernum, sub);
        return result;
    }
